...
const deployIcoFactoryCtrl = {
	...
	createDeployIco: async (req, res) => {
		try {
			...
			if (newDeployed) {
				return res
					.status(201)
					.json({ msg: "Deploy New Ico is created successfully!" });
			}
	...
	updateDeployIco: async (req, res) => {
		try {
			...
			const updatedDeployIco = await deployIcoFactory.updateOne(
				{ _id: token_id },
				{
			...
			if (updatedDeployIco) {
				return res
					.status(201)
					.json({
						msg: "Value of tier One, Two and Three are updated successfully!",
					});
			}
	...
	deleteDeployIco: async (req, res) => {
		try {
			const token_id = req.params._id;
			const deleteToken = await deployIcoFactory.findOneAndRemove({
				_id: token_id,
			});
			if (deleteToken) {
				return res
					.status(201)
					.json({ msg: "Deployed New Ico is deleted successfully!" });
			}
