			...
			<div className="pool-link">
				<div className="tit_le">
					<div className="title-img">
						<a key={pool._id} href={`/pool_detail/featured/${pool._id}`}>
					...
					<div className="title-head">
						<h3>
							<div className="h-title">
								<Link key={pool._id} to={`/pool_detail/featured/${pool._id}`}>
									{title_length
										? pool.title.substring(0, 20) + "..."
										: pool.title}
								</Link>
							</div>
							<span>
						...
						<div className="title-info">
							<p>
								{length ? pool.content.substring(0, 170) : pool.content}
								{length ? "..." : ""}
								{length ? (
									<a href="#read-more" onClick={handleOpen}>
										{" "}
										Read More
									</a>
								) : (
				...
				<div className="prog_bar">
					<div className="prog_bar_grd">
						...
						<span className="parti">
							Max Participants{" "}
							<span className="white_text">{pool.participants}</span>
						</span>
					</div>
					...
					<div className="prog_bar_grd">
						{
							<span className="prog _percent">
								{number1 ? number1.toFixed(2) : "0"}%
							</span>
						}
						{
							<span className="parti _nls">
								{allocation ? allocation.toFixed(2) : "0"}/{pool.total_supply}{" "}
								{pool.symbol}
							</span>
						}
					</div>
...
export default memo(FeaturedPool);
